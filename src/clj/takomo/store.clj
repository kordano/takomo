...
(defonce store (atom {:uri  "datahike:mem://takomo"
                      :conn nil}))
...
(defn init []
  (let [uri (:uri @store)
    ...
    (swap! store assoc :conn (d/connect uri))))
...
(defn create-member [{:keys [:member/password] :as new-member}]
  (d/transact! (:conn @store) [(-> new-member
...
(defn read-member-by-id [id]
  (d/pull (d/db (:conn @store)) '[:db/id :member/firstname :member/lastname :member/email] id))
...
(defn create-customer [new-customer]
  (d/transact! (:conn @store) [(select-keys new-customer customer-keys)]))
...
(defn read-customer-by-id [id]
  (d/pull (d/db (:conn @store)) '[:db/id :customer/name
                                  :customer/contact
                                  :customer/department
                                  :customer/postal
                                  :customer/city
...
(defn update-customer [{:keys [:db/id] :as customer}]
  (if-not id
    ...
    (d/transact! (:conn @store) [(-> customer
